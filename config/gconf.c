...
static void text_insert_help(struct menu *menu)
{
    ...
    gtk_text_buffer_insert_with_tags(buffer, &end, prompt, -1, tag1, NULL);
...
static void text_insert_msg(const char *title, const char *message)
{
    ...
    gtk_text_buffer_get_end_iter(buffer, &end);
    gtk_text_buffer_insert_with_tags(buffer, &end, title, -1, tag1, NULL);
    gtk_text_buffer_insert_at_cursor(buffer, "\n\n", 2);
    gtk_text_buffer_get_end_iter(buffer, &end);
    gtk_text_buffer_insert_with_tags(buffer, &end, msg, -1, tag2, NULL);
}
                                 ...
                                 gpointer user_data)
{
    ...
    dialog = gtk_dialog_new_with_buttons(_("Warning !"),
                                         GTK_WINDOW(main_wnd), (GtkDialogFlags)
                                         (GTK_DIALOG_MODAL |
...
static void load_filename(GtkFileSelection * file_selector, gpointer user_data)
{
    ...
    fn = gtk_file_selection_get_filename(GTK_FILE_SELECTION(user_data));
...
static gchar **fill_row(struct menu *menu)
{
    ...
    if (sym_is_choice(sym)) {   // parse childs for getting final value
        ...
        if (def_menu)
            row[COL_VALUE] = g_strdup(_(menu_get_prompt(def_menu)));
    }
...
static void set_node(GtkTreeIter * node, struct menu *menu, gchar ** row)
{
    ...
    gtk_tree_store_set(tree, node,
                       ...
                       COL_BTNRAD, GPOINTER_TO_INT(row[COL_BTNRAD]), -1);
...
static void update_tree(struct menu *src, GtkTreeIter * dst)
{
 ...
 reparse:
        menu1 = child1;
        if (valid)
            gtk_tree_model_get(model2, child2, COL_MENU, &menu2, -1);
        else
        ...
        if (menu1 != menu2) {
            if (gtktree_iter_find_node(dst, menu1) == NULL) {   // add node
                if (!valid && !menu2)
                    ...
                    sibling = child2;
                gtk_tree_store_insert_before(tree2, child2, dst, sibling);
...
int main(int ac, char *av[])
{
    ...
    if (env)
        ...
        glade_file =
            g_strconcat(g_get_current_dir(), "/", av[0], ".glade", NULL);
