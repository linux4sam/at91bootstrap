...
static
FATFS *FatFs[] = { NULL };      /* Pointer to the file system objects (logical drives) */
    ...
    )
{
    ...
    fsect = fs->fatbase;
    switch (fs->fs_type) {
#if 0                           //this FS format is no longer used in most of systems
    case FS_FAT12:
        bc = clst;
        bc += bc / 2;
...
#if _FS_MINIMIZE <= 1
static
FRESULT dir_read(DIR * dj       /* Pointer to the directory object that pointing the entry to be read */
    )
{
    FRESULT res;
    ...
    *path = &p[si];             /* Rerurn pointer to the next segment */
    ...
    for (;;) {
        ...
        if (i >= ni || si == di) {      /* Extension or end of SFN */
            ...
            if (si != di)
                cf |= NS_LOSS | NS_LFN; /* File name is longer than 8 bytes */
            if (si > di)
                break;          /* No extension */
            si = di;
            i = 8;
        ...
        if (w >= 0x80) {        /* Non ASCII char */
            ...
            if (w)
                w = cvt[w - 0x80];      /* Convert extend char (SBCS) */
#else
    ...
    if (dj->fn[0] == 0xE5)
        dj->fn[0] = 0x05;       /* If the first char collides with 0xE5, replace it with 0x05 */
    ...
    )
{
    ...
    if (!memcmp(&fs->win[BS_FilSysType], fatstr, 3))    /* Check FAT signature */
        return 0;
    if (!memcmp(&fs->win[BS_FilSysType32], fatstr, 3)
