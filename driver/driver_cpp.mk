# Memory support

# RAM type

ifeq ($(CONFIG_SDRAM),y)
CPPFLAGS += -DCONFIG_SDRAM
endif

ifeq ($(CONFIG_SDDRC),y)
CPPFLAGS += -DCONFIG_SDDRC
endif

ifeq ($(CONFIG_DDR2),y)
CPPFLAGS += -DCONFIG_DDR2
endif

ifeq ($(CONFIG_LPDDR1),y)
CPPFLAGS += -DCONFIG_LPDDR1
endif

###   Support of known memory chip
# ISSI IS43LR32400
ifeq ($(CONFIG_RAM_CHIP_IS43LR32400),y)
CPPFLAGS += -DCONFIG_RAM_CHIP_IS43LR32400
endif
# Winbond WD948D2 (256Mb)
ifeq ($(CONFIG_RAM_CHIP_W948D2),y)
CPPFLAGS +=-DCONFIG_RAM_CHIP_W948D2
endif

##If no external RAM
ifeq ($(CONFIG_ONLY_INTERNAL_RAM),y)
CPPFLAGS += -DCONFIG_ONLY_INTERNAL_RAM
endif

# Support for PSRAM on SAM9263EK EBI1

ifeq ($(CONFIG_PSRAM),y)
CPPFLAGS += -DCONFIG_PSRAM
endif

# 16 bit operation

ifeq ($(CONFIG_SDRAM_16BIT),y)
CPPFLAGS += -DCONFIG_SDRAM_16BIT
endif

# SDRAM/DDR/DDR2 size

ifeq ($(CONFIG_RAM_16MB),y)
CPPFLAGS += -DCONFIG_RAM_16MB -DCONFIG_RAM_SIZE=16
endif

ifeq ($(CONFIG_RAM_32MB),y)
CPPFLAGS += -DCONFIG_RAM_32MB -DCONFIG_RAM_SIZE=32
endif

ifeq ($(CONFIG_RAM_64MB),y)
CPPFLAGS += -DCONFIG_RAM_64MB -DCONFIG_RAM_SIZE=64
endif

ifeq ($(CONFIG_RAM_128MB),y)
CPPFLAGS += -DCONFIG_RAM_128MB -DCONFIG_RAM_SIZE=128
endif

ifeq ($(CONFIG_RAM_256MB),y)
CPPFLAGS += -DCONFIG_RAM_256MB -DCONFIG_RAM_SIZE=256
endif

ifeq ($(CONFIG_RAM_512MB),y)
CPPFLAGS += -DCONFIG_RAM_512MB -DCONFIG_RAM_SIZE=512
endif

# Boot flash type

ifeq ($(CONFIG_DATAFLASH),y)
CPPFLAGS += -DCONFIG_DATAFLASH
endif

ifeq ($(CONFIG_NANDFLASH),y)
CPPFLAGS += -DCONFIG_NANDFLASH
endif

ifeq ($(CONFIG_SDCARD),y)
CPPFLAGS += -DCONFIG_SDCARD
endif

ifeq ($(CONFIG_FLASH),y)
CPPFLAGS += -DCONFIG_FLASH
endif

ifeq ($(CONFIG_LOAD_LINUX),y)
CPPFLAGS += -DCONFIG_LOAD_LINUX
endif

ifeq ($(CONFIG_LOAD_ANDROID),y)
CPPFLAGS += -DCONFIG_LOAD_ANDROID
endif

ifeq ($(CONFIG_LINUX_UIMAGE), y)
CPPFLAGS += -DCONFIG_LINUX_UIMAGE
endif

ifeq ($(CONFIG_LINUX_ZIMAGE), y)
CPPFLAGS += -DCONFIG_LINUX_ZIMAGE
endif

ifeq ($(CONFIG_SDCARD_HS),y)
CPPFLAGS += -DCONFIG_SDCARD_HS
endif

ifeq ($(CONFIG_OF_LIBFDT),y)
CPPFLAGS += -DCONFIG_OF_LIBFDT
endif

# Dataflash support
ifeq ($(CONFIG_DATAFLASH_RECOVERY),y)
CPPFLAGS += -DCONFIG_DATAFLASH_RECOVERY
endif

ifeq ($(CONFIG_SMALL_DATAFLASH),y)
CPPFLAGS += -DCONFIG_SMALL_DATAFLASH
endif

ifeq ($(MEMORY),dataflash)
CPPFLAGS += -DAT91C_SPI_CLK=$(SPI_CLK)
CPPFLAGS += -DAT91C_SPI_PCS_DATAFLASH=$(SPI_BOOT) 
endif

ifeq ($(MEMORY),dataflashcard)
CPPFLAGS += -DAT91C_SPI_CLK=$(SPI_CLK)
CPPFLAGS += -DAT91C_SPI_PCS_DATAFLASH=$(SPI_BOOT) 
endif

# NAND flash support

ifeq ($(CONFIG_NANDFLASH_SMALL_BLOCKS),y)
CPPFLAGS += -DCONFIG_NANDFLASH_SMALL_BLOCKS
endif

ifeq ($(CONFIG_ENABLE_SW_ECC), y)
CPPFLAGS += -DCONFIG_ENABLE_SW_ECC
endif

ifeq ($(CONFIG_USE_PMECC), y)
CPPFLAGS += -DCONFIG_USE_PMECC
endif

ifeq ($(CONFIG_ON_DIE_ECC), y)
CPPFLAGS += -DCONFIG_ON_DIE_ECC
endif

ifeq ($(CONFIG_NANDFLASH_RECOVERY),y)
CPPFLAGS += -DCONFIG_NANDFLASH_RECOVERY
endif

ifeq ($(CONFIG_PMECC_CORRECT_BITS_2), y)
CPPFLAGS += -DPMECC_ERROR_CORR_BITS=2
endif

ifeq ($(CONFIG_PMECC_CORRECT_BITS_4), y)
CPPFLAGS += -DPMECC_ERROR_CORR_BITS=4
endif

ifeq ($(CONFIG_PMECC_CORRECT_BITS_8), y)
CPPFLAGS += -DPMECC_ERROR_CORR_BITS=8
endif

ifeq ($(CONFIG_PMECC_CORRECT_BITS_12), y)
CPPFLAGS += -DPMECC_ERROR_CORR_BITS=12
endif

ifeq ($(CONFIG_PMECC_CORRECT_BITS_24), y)
CPPFLAGS += -DPMECC_ERROR_CORR_BITS=24
endif

ifeq ($(CONFIG_PMECC_SECTOR_SIZE_512), y)
CPPFLAGS += -DPMECC_SECTOR_SIZE=512
endif

ifeq ($(CONFIG_PMECC_SECTOR_SIZE_1024), y)
CPPFLAGS += -DPMECC_SECTOR_SIZE=1024
endif

ifeq ($(CONFIG_ONFI_DETECT_SUPPORT), y)
CPPFLAGS += -DCONFIG_ONFI_DETECT_SUPPORT
endif

ifeq ($(CONFIG_USE_ON_DIE_ECC_SUPPORT), y)
CPPFLAGS += -DCONFIG_USE_ON_DIE_ECC_SUPPORT
endif

# Debug related stuff
ifeq ($(CONFIG_DEBUG_ERROR),y)
CPPFLAGS += -DBOOTSTRAP_DEBUG_LEVEL=DEBUG_ERROR
endif

ifeq ($(CONFIG_DEBUG_INFO),y)
CPPFLAGS += -DBOOTSTRAP_DEBUG_LEVEL=DEBUG_INFO
endif

ifeq ($(CONFIG_DEBUG_LOUD),y)
CPPFLAGS += -DBOOTSTRAP_DEBUG_LEVEL=DEBUG_LOUD
endif

ifeq ($(CONFIG_DEBUG_VERY_LOUD),y)
CPPFLAGS += -DBOOTSTRAP_DEBUG_LEVEL=DEBUG_VERY_LOUD
endif

ifeq ($(CONFIG_DISABLE_WATCHDOG),y)
CPPFLAGS += -DCONFIG_DISABLE_WATCHDOG
endif

ifeq ($(CONFIG_LONG_FILENAME), y)
CPPFLAGS += -DCONFIG_LONG_FILENAME
endif

ifeq ($(CONFIG_DATAFLASH_LOAD_WITH_DMA), y)
CPPFLAGS += -DCONFIG_DATAFLASH_LOAD_WITH_DMA
endif

## EBI memory specifications
ifeq ($(CONFIG_EBI_0_USED), y)
CPPFLAGS += -DCONFIG_EBI_0_SIZE=$(CONFIG_EBI_0_SIZE)
else
CPPFLAGS += -DCONFIG_EBI_0_SIZE=0
endif

ifeq ($(CONFIG_EBI_1_USED), y)
CPPFLAGS += -DCONFIG_EBI_1_SIZE=$(CONFIG_EBI_1_SIZE)
else
CPPFLAGS += -DCONFIG_EBI_1_SIZE=0
endif

ifeq ($(CONFIG_EBI_2_USED), y)
CPPFLAGS += -DCONFIG_EBI_2_SIZE=$(CONFIG_EBI_2_SIZE)
else
CPPFLAGS += -DCONFIG_EBI_2_SIZE=0
endif

ifeq ($(CONFIG_EBI_3_USED), y)
CPPFLAGS += -DCONFIG_EBI_3_SIZE=$(CONFIG_EBI_3_SIZE)
else
CPPFLAGS += -DCONFIG_EBI_3_SIZE=0
endif

ifeq ($(CONFIG_SET_PARAMS_SPI_FLASH),y)
CPPFLAGS += -DCONFIG_SET_PARAMS_SPI_FLASH
endif

ifneq ($(strip $(CONFIG_SPI_FLASH_PARAMS_PAGES)),)
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGES=$(CONFIG_SPI_FLASH_PARAMS_PAGES)
endif

ifeq ($(CONFIG_SPI_FLASH_PARAMS_PAGE_SIZE_IS_POWER2),y)
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_SIZE_IS_POWER2=1
else
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_SIZE_IS_POWER2=0
endif

ifneq ($(strip $(CONFIG_SPI_FLASH_PARAMS_PAGE_SIZE)),)
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_SIZE=$(CONFIG_SPI_FLASH_PARAMS_PAGE_SIZE)
else
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_SIZE=0
endif

ifneq ($(strip $(CONFIG_SPI_FLASH_PARAMS_PAGE_OFFSET)),)
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_OFFSET=$(CONFIG_SPI_FLASH_PARAMS_PAGE_OFFSET)
else
CPPFLAGS += -DCONFIG_SPI_FLASH_PARAMS_PAGE_OFFSET=0
endif
	
ifeq ($(CONFIG_EXTERNAL_RAM_TEST_INFINITE),y)
CPPFLAGS += -DCONFIG_EXTERNAL_RAM_TEST_INFINITE
endif

ifeq ($(CONFIG_EXTERNAL_RAM_TEST_WITHOUT_BURST),y)
CPPFLAGS += -DCONFIG_EXTERNAL_RAM_TEST_WITHOUT_BURST
endif

ifeq ($(CONFIG_EXTERNAL_RAM_TEST_WITH_BURST),y)
CPPFLAGS += -DCONFIG_EXTERNAL_RAM_TEST_WITH_BURST
endif
